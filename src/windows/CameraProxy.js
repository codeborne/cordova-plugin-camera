  ...
  file.properties.getImagePropertiesAsync().then(function (imageProps) {
    file.copyAsync(storageFolder, file.name, Windows.Storage.NameCollisionOption.replaceExisting)
      ...
      .then(function (buffer) {
        ...
        image.src = base64imageDataFromBytes(buffer, contentType);
        ...
        image.onload = function () {
          ...
          resizedImage.onload = function () {
            var fileContent = rotatedImageContent(this, canvas, targetWidth, targetHeight, imageProps.orientation, contentType);
...
function base64imageDataFromBytes(buffer, contentType) {
  var strBase64 = encodeToBase64String(buffer);
  return "data:" + contentType + ";base64," + strBase64;
}

function rotatedImageContent(image, canvas, targetWidth, targetHeight, orientation, targetContentType) {
  ...
  ctx.rotate(degrees(orientation));
  ctx.drawImage(image, image.width / 2 - targetWidth / 2, 0, targetWidth, targetHeight, -canvas.width / 2, -canvas.height / 2, targetWidth, targetHeight);
...
function resizedImageCanvas(image, targetWidth, targetHeight) {
  ...
  var imageHeight = ratio * image.height;
